...
let currentIndex = 0;
  ...
  // Add event listeners to each farm icon
document.querySelectorAll('.farm-icon').forEach(icon => {
  icon.addEventListener('click', () => {
    // Get the layout name from the clicked icon's data-layout attribute
    const layout = icon.getAttribute('data-layout');
    const farmImage = document.getElementById('farm-image');
    
    // Change the displayed image based on the layout clicked
    switch(layout) {
      case 'layout1':
        farmImage.src = 'images/farm-layout1.jpg';  // Update with the correct image path
        farmImage.alt = 'Farm Layout 1';
        break;
      case 'layout2':
        farmImage.src = 'images/farm-layout2.jpg';  // Update with the correct image path
        farmImage.alt = 'Farm Layout 2';
        break;
  ...
  const icons = document.querySelectorAll('.farm-icon');
